import { AssetOrder, AssetOrderBy, getAssetInfo, type AssetResponseDto } from '$lib/api/compat';
...
export class TimelineManager extends VirtualScrollManager {
    ...
    this.#unsubscribes.push(
      eventManager.on({
        ...
        AssetsUpload: (files) => {
          ...
          this.upsertAssets(files.map((file) => mediaStore.fileToTimelineAsset(file)));
        },
      }),
    );
  }
  ...
  *timelineMonthIterator(options?: { direction?: Direction; startTimelineMonth?: TimelineMonth }) {
  ...
  async #initializeTimelineMonths() {
    const timebuckets = await mediaStore.getMonthBuckets();
  ...
  /** Force a full data refetch + rebuild (pull-to-refresh). */
  async reload() {
  ...
  async findTimelineMonthForAsset(asset: AssetDescriptor | AssetResponseDto) {
    ...
    const response = isAssetResponseDto(asset)
      ? asset
      : await getAssetInfo({ id }).catch(() => null);
    if (!response) {
      return;
    }
