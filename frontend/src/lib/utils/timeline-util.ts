import { AssetTypeEnum, AssetOrderBy, type AssetResponseDto } from '$lib/api/compat';
...
export const toTimelineAsset = (unknownAsset: AssetResponseDto | TimelineAsset): TimelineAsset => {
  ...
  return {
    ...
    tags: assetResponse.tags?.map((tag: { id: string }) => tag.id),
    ratio,
    ...
    stack: assetResponse.stack ? { id: assetResponse.stack.id, assetCount: assetResponse.stack.assets.length } : null,
